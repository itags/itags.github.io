...
module.exports = function (window) {
    ...
    if (!window.ITAGS[itagName]) {
        ISuperClass = require('i-button')(window);  // <-- define the itag-name of the superClass here NOT by variable, for browserify wouldn't load it

        Itag = ISuperClass.subClass(itagName, {
            attrs: {
                disabled: 'boolean',
                type: 'string'
            },
